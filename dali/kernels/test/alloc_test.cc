...
TEST(KernelAlloc, AllocFree) {
  ...
  for (int i = 0; i < static_cast<int>(AllocType::Count); i++) {
    ...
    memory::GetDeallocator(alloc)(mem);
...
TEST(KernelAlloc, HostDevice) {
  ...
  auto pinned_deallocator = memory::GetDeallocator(AllocType::Pinned);
  auto host_deallocator = memory::GetDeallocator(AllocType::Host);
  auto gpu_deallocator = memory::GetDeallocator(AllocType::GPU);
