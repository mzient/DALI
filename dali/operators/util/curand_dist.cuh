...
#ifndef DALI_OPERATORS_UTIL_CURAND_DIST_CUH_
#define DALI_OPERATORS_UTIL_CURAND_DIST_CUH_
...
namespace dali {
  ...
  struct curand_uniform_dist {
    ...
    DALI_HOST_DEV curand_uniform_dist(T start, T end)
      ...
      min_value_ = range_start_;
      max_value_ = nextafter(range_end_, T(range_start_));
      if (min_value_ > max_value_) {
        std::swap(min_value_, max_value_);
      }
    }

    template <typename RNG>
    __device__ inline T operator()(RNG &state) const {
      T val;
      if (std::is_same_v<T, double>) {
        do {
          val = fma(curand_uniform_double(&state), range_size_, range_start_);
        } while ((val - range_end_) * range_size_ > 0);
      } else {
        do {
          val = fmaf(curand_uniform(&state), range_size_, range_start_);
        } while ((val - range_end_) * range_size_ > 0);
      }
      return val;
    }

    __device__ inline x4output get4(curandPhilox4_32_10_t &state) const {
      x4output val;
      if (std::is_same_v<T, double>) {
        val = fma(curand_uniform4_double(&state), range_size_, range_start_);
      } else {
        val = fma(curand_uniform4(&state), range_size_, range_start_);
      }
      return val;
    }

    using x4output = std::conditional_t<std::is_same_v<T, double>, double4, float4>;
   private:
    T range_start_ = 0, range_end_ = 1, range_size_ = 1;
    T min_value_ = range_start_;
    T max_value_ = nextafter(range_end_, T(range_start_));
  };
...
#endif  // DALI_OPERATORS_UTIL_CURAND_DIST_CUH_
